...
|---|---|---|
| `--config <path>` | full, dns, route, ip, headers, tech, config show | Override config file lookup |
| `--output text\|json\|markdown\|html` | full, dns, route, ip, headers, tech, config show | Output format |
| `--out <file>` | full, dns, route, ip, headers, tech, config show | Write to file instead of stdout |
| `--timeout <duration>` | full, dns, route, ip, headers, tech | Per-check or overall timeout (semantic varies by command, documented in `--help`) |
...
`full` (default), `dns`, `route`, `ip`, `headers`, `tech`, `app`, `menu`, `config show`, `help`, `version` — all stable.
...
The `"kind"` discriminator (`"full"` / `"dns"` / `"route"` / `"ip"` / `"headers"` / `"tech"` / `"config"`) is stable.
