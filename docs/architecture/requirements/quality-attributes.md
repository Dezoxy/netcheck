...
| QA-02 | Portability | A release is cut. | Working binaries for macOS, Linux and Windows on amd64 and arm64, plus a multi-arch (linux/amd64, linux/arm64) container image. | The CI cross-compile matrix and its `version` smoke test (`.github/workflows/ci.yml`); goreleaser builds. | All six targets build on every pull request. | `route` does not work inside the container image (TD-002). |
