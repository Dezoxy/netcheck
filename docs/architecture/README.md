...
| ContainerDeployment | CTO, operators | Where does netcheck run when started from the GHCR image? | Container environment | `include *` | Platform detail (the image is multi-arch) | `Dockerfile`, `.goreleaser.yml` dockers_v2 | Image, base, user or bind change | Checked 2026-09-21 (PNG, 2026.09.19): readable, no overlaps |
