...
func RunWhois(args []string) int {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, "Looks up the sponsoring registrar for <domain> via RDAP (the modern,")
		fmt.Fprintln(os.Stderr, "structured replacement for WHOIS). Reports the registrar name, its IANA")
		fmt.Fprintln(os.Stderr, "ID, and URL. Passive — only talks to the RDAP service, never the target.")
		fmt.Fprintln(os.Stderr)
		fmt.Fprintln(os.Stderr, "Registrar fields follow the ICANN gTLD RDAP profile; many ccTLDs (.hu and")
		fmt.Fprintln(os.Stderr, "other WHOIS-only registries) expose no RDAP registrar data.")
		fmt.Fprintln(os.Stderr)
